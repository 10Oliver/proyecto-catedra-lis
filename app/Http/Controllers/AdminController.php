...
class AdminController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.dashboard', compact(
            'totalEmpresas', 'totalUsuarios', 'totalCupones', 'meses', 'ganancias'
        ));
    }

    /**
     * Solicitudes: empresas sin usuario asociado
     */
    public function empresas()
{
    // Solicitudes: empresas que aún no tienen un usuario creado
    ...
    /**
     * Aprueba la empresa y guarda el porcentaje de comisión
     */
    public function aprobarEmpresa(Request $request, Company $company)
{
    ...
    return redirect()
        ...
        ->with('success', 'Empresa aprobada correctamente.');
}
...
public function updateCompanyUser(Request $request, Company $company, User $user)
{
    ...
    public function rechazarEmpresa(Company $company)
    {
        ...
        return redirect()
            ->route('admin.empresas')
            ->with('success', 'Empresa rechazada correctamente.');
    }

    /**
     * Muestra el formulario para crear el usuario de la empresa
     */
    ...
    /**
     * Almacena el nuevo usuario de la empresa aprobada
     */
    public function storeCompanyUser(Request $request, Company $company)
    {
        ...
        $company->users()->attach($user->user_uuid, [
            'approved_uuid' => Auth::user()->user_uuid,
        ]);

        return redirect()
            ->route('admin.empresas')
            ->with('success', 'Usuario creado y asociado a la empresa.');
    }
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'dui'                   => 'required|string|max:10|unique:user,dui',
            'birthdate'             => 'required|date',
